...
public class MyLogger {
    ...
    public static void setup() throws IOException {
        ...
        FileHandler logfile;
        try {
            logfile = new FileHandler("log/levelBuilder_"+date+".log"); //we try to create the log file
        } catch (final NoSuchFileException e) { //if the log folder doesn't exist
            new File("log").mkdirs(); //we create the folder
            logfile = new FileHandler("log/levelBuilder_"+date+".log"); //and create the log file
        }
