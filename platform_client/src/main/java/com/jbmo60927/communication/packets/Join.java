...
public class Join extends Packet {
    ...
    protected Join(Parameter[] parameters, App app) {
    ...
    private static final Parameter[] setParameters(App app) {
        return new Parameter[] {
            new Parameter(Welcome.TYPES.findType("name"), stringToBytes(app.getConnect().getGameLinkThread().getName())),
            new Parameter(Welcome.TYPES.findType("version"), stringToBytes(App.VERSION))
        };
    }
